...
def downsampling_factor(n_frames, gtscore):
    downsample_factor = n_frames/gtscore.shape[0]
    downsample_factor = int(np.ceil(downsample_factor))
...
def train(args, split, save_path):
    ...
    for epoch in range(args.max_epoch):
        ...
        for _, seq, gtscore, change_points, n_frames, nfps, picks, _,vid_name in train_loader:
            ...
            downsample_factor = downsampling_factor(n_frames, gtscore)
            target = vsumm_helper.downsample_summ(keyshot_summ, downsample_factor)
            ...
            # print(f'  vid: {vid_name}, loss_cls: {stats.cls_loss}, loss_loc: {stats.loc_loss}, str_loss: {stats.ctr_loss}, loss: {stats.loss}')
