...
# python sum_ind/main.py -d ../../datasets/fvs.h5 -s ../../splits/fvs_splits.json -m tvsum --split-id 3 --gpu 3 --save-dir sum-ind_log3
...
parser.add_argument('--save-dir', type=str, default='sum-ind_log0', help="path to save output (default: 'log/')")
...
def main():
    ...
    model = SumInd(in_dim=args.input_dim, hid_dim=args.hidden_dim, num_layers=args.num_layers)
    ...
    for idx in np.arange(len(train_keys)):
        key = train_keys[idx]
        seq = dataset[key]['features'][...]  # sequence of features, (seq_len, dim)
        picks = dataset[key]['picks']
        # get segment boundaries
        frames_per_segment = dataset[key]['n_frame_per_seg']
        # get frames of picks
        frame_idxs[key] = map_selected_frames_to_idxs(picks, seq)
        # get frame ids pro segment
        segments[key] = get_frames_per_segments(frames_per_segment, picks)
        segment_centers[key] = (get_mean_frames_of_segments(segments[key]))
        # print(len(segment_centers[key]),len(segments[key].keys()))

    # save_pickle_file('summe_segments_centers', segment_centers)
    # save_pickle_file('summe_segments', segments)

    #segment_centers = open_pickle_file('summe_segments_centers.pickle')
    #segments = open_pickle_file('summe_segments.pickle')
    ...
    for epoch in range(start_epoch, args.max_epoch):
        ...
        for idx in idxs:
            ...
            for _ in range(args.num_episode):
                ...
                reward = compute_reward(seq, actions, segment_centers[key], segments[key],
                                        use_gpu=use_gpu)
...
def evaluate(model, dataset, test_keys, use_gpu):
    ...
    with torch.no_grad():
        ...
        for key_idx, key in enumerate(test_keys):
            ...
            ## save to all_summaries
            all_summaries.append(machine_summary)
            fm, precision, recall = vsum_tools.evaluate_summary(machine_summary, user_summary, eval_metric)
            fms.append(fm)
            ...
            if args.save_results:
                ...
                h5_res.create_dataset(key + '/precision', data=precision)
                h5_res.create_dataset(key + '/recall', data=recall)
        # save all summaries as pkl
        SUMMARY_DIR = 'summaries/split' + str(args.split_id)
        os.makedirs(SUMMARY_DIR, exist_ok=True)
        with open(f'{SUMMARY_DIR}/summaries.pkl', 'wb') as file:
            pickle.dump(all_summaries, file)
        print(f'- Summaries saved to {SUMMARY_DIR}/summaries.pkl')
    ...
    print("Average Precision {:.1%}".format(np.mean(precisions)))
    print("Average Recall {:.1%}".format(np.mean(recalls)))
